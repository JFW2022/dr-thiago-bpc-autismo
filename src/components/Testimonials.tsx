...
import { Star, MessageSquareQuote, ShieldCheck, FileSearch } from 'lucide-react';
  ...
  return (
    <section id="depoimentos" className="py-16 md:py-24 bg-white border-t border-slate-200 overflow-hidden">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Testimonials Grid: Clean Cards showcasing Full Prints */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12 items-stretch">
          {testimonials.map((item, index) => (
            <div
              key={index}
              className={`bg-brand-warmBg rounded-3xl border border-slate-200 p-6 sm:p-7 shadow-lg flex flex-col justify-between h-full hover:border-brand-amber/50 transition-all duration-300 ${item.revealClass}`}
            >
              {/* Stars Header */}
              ...
              {/* Footer Info */}
              <div className="mt-5 flex items-center justify-between text-xs text-slate-500 pt-4 border-t border-slate-200/80">
